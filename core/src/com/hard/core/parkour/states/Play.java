...
import com.hard.core.parkour.entities.HUD;
...
public class Play extends GameState {
   ...
   //private BitmapFont font = new BitmapFont();
    ...
    private void switchBlocks(){
        ...
        Filter filter = this.player.getBody().getFixtureList().first().getFilterData();
        ...
        if((bits & B2DVars.BIT_RED) != 0){
            ...
            bits |= B2DVars.BIT_GREEN | B2DVars.BIT_CRYSTAL;
        }
        else if((bits & B2DVars.BIT_GREEN) != 0) {
            bits &= ~B2DVars.BIT_GREEN;
            bits |= B2DVars.BIT_BLUE | B2DVars.BIT_CRYSTAL;
        }
        else if((bits & B2DVars.BIT_BLUE) != 0) {
            bits &= ~B2DVars.BIT_BLUE;
            bits |= B2DVars.BIT_RED | B2DVars.BIT_CRYSTAL;
        }

        //set new mask buts to player
        filter.maskBits = bits;
        this.player.getBody().getFixtureList().first().setFilterData(filter);
